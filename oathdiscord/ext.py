...
class OauthAppCommandPermissions: 
    def __init__(self, id: Snowflake, type: int, permission: bool):
        ...
        """
        self.id = str(id)
        if (not type in [1,2,3]): raise ValueError('Invalid value for "type" argument.')
        self.type = type
...
class Bot(commands.Bot):
    ...
    async def edit_app_command_perms(self, guild_id: str, command_id: str, permissions: list[OauthAppCommandPermissions]):
        url = self._base_url % (f"{self.client_id}/guilds/{guild_id}/commands/{command_id}/permissions")
        ...
        return OauthAppConnections(**request_json) if request.status == 200 else request_json
